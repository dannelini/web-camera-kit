export interface CameraKitSettings {
  recordVideoFrameRate: number;
  recordVideoBitsPerSecond: number;
  recordAudioBitsPerSecond: number;
  recordedFileName: string;
  snapshotFileName: string;
  recordMicrophoneAudio: boolean;
  recordLensAudio: boolean;
  processVideoWithFFmpeg: boolean;
  recodeVideoCodecs: Array<{
    codecString: string;
    container: string;
  }>;
  defaultCameraType: 'BACK' | 'FRONT';
  showDebugLog: boolean;
}

const settings: CameraKitSettings = {
